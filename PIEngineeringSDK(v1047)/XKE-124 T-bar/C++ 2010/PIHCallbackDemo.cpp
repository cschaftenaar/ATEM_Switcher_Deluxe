...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_CHECK2:
			...
			if (hList == NULL) return TRUE;
			if (SendMessage(hList, BM_GETCHECK, 0, 0)==BST_CHECKED) buffer[3]=1; //0=off, 1=on
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1275)
				{
					AddEventMsg(hDialog, "Found Device: XKE-124 T-bar, PID=1275 (PID #1)");
				}
				else if (pid==1276)
				{
					AddEventMsg(hDialog, "Found Device: XKE-124 T-bar, PID=1276 (PID #2)");
				}
				else if (pid==1277)
				{
					AddEventMsg(hDialog, "Found Device: XKE-124 T-bar, PID=1277(PID #3)");
				}
				else if (pid==1278)
				{
					AddEventMsg(hDialog, "Found Device: XKE-124 T-bar, PID=1278 (PID #4)");
				}
				else
				{
