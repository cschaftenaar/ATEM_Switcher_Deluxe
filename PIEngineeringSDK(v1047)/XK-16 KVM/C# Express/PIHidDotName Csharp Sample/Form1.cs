                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc)
                    {
                        switch (devices[i].Pid)
                        {
                            case 1269:
                                ...
                                CboDevices.Items.Add("XK-16 KVM (" + devices[i].Pid + "=PID #1)");
                    ...
                    else
                    {
                        if (devices[i].Pid == 1270)
                        {
                            ...
                            CboDevices.Items.Add("XK-16 KVM (" + devices[i].Pid + "=PID #2), no reading or writing available, change to PID #1 by sliding program switch up and replugging device.");
                            cbotodevice[cbocount] = i;
                            cbocount++;
                            DisableAllControls();
                        }
