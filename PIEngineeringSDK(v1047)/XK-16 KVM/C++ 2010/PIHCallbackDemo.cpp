	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1235)
				{
					AddEventMsg(hDialog, "Found Device: X-keys XK-16 KVM, PID=1269 or PID #1");
				}
				
				return;
			}
		}
		else if (info[i].UP == 0xC)    
		{	
			hnd = info[i].Handle;
			result = SetupInterfaceEx(hnd);
			if (result != 0)    
		...
		else
		{
				if (pid==1269)
				{
					AddEventMsg(hDialog, "Found Device: X-keys XK-16 KVM, PID=1269 or PID #2 (KVM).");
