...
namespace PIHidDotName_Csharp_Sample
{
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        private void DisableAllControls()
        {
            ...
            MessageBox.Show("Device in PID #2 (KVM), no input or output reports are available.  To change to PID #1 replug device into usb port and quickly press Scroll Lock 10-15 times.");
        }
        ...
        private void BtnMousereflect_Click(object sender, EventArgs e)
        {
            if (CboDevices.SelectedIndex != -1) //do nothing if not enumerated
            {
                ...
                wData[5] = 0;
                wData[6] = Convert.ToByte(this.TxtMouseWheel.Text); //wheel Y 128=0 no motion, 1-127 is up, 255-129=down, finest inc (1 and 255) to coarsest (127 and 129)
