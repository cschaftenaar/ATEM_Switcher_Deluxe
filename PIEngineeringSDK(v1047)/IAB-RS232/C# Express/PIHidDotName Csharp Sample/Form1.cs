            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc)// && devices[i].WriteLength==36)
                    {
                        ...
                        {
                            case 1257:
                                CboDevices.Items.Add("IAB-RS232(" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1258:
                                CboDevices.Items.Add("IAB-RS232(" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1259:
                                CboDevices.Items.Add("IAB-RS232(" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1260:
                                CboDevices.Items.Add("IAB-RS232(" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            default:
