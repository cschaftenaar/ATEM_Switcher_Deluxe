...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_BAUD:
			{
				//Sending this command will change the baud rate of the IAB-RS232, it must match that of the connected serial device
				for (int i=0;i<wlen;i++)
				{
					buffer[i]=0;
				}
		...
		case IDC_PARITY:
			{
				//Sending this command will change the parity of the IAB-RS232, it must match that of the connected serial device
				for (int i=0;i<wlen;i++)
				{
					buffer[i]=0;
				}
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1257)
				{
					AddEventMsg(hDialog, "Found Device: IAB-RS232, PID=1257 (PID #1)");
				}
				else if (pid==1258)
				{
					AddEventMsg(hDialog, "Found Device: IAB-RS232, PID=1258 (PID #2)");
				}
				else if (pid==1259)
				{
					AddEventMsg(hDialog, "Found Device: IAB-RS232, PID=1259 (PID #3)");
				}
				else if (pid==1260)
				{
					AddEventMsg(hDialog, "Found Device: IAB-RS232, PID=1260 (PID #4)");
				}
				else
				{
