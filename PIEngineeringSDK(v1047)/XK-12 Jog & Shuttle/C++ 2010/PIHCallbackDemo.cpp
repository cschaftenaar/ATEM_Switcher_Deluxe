	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1062)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Jog & Shuttle, PID=1062 or PID #1");
				}
				else if (pid==1063)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Jog & Shuttle, PID=1063");
				}
				else if (pid==1064)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Jog & Shuttle, PID=1064 or PID #2");
				}
				else
				{
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
		...
		char dataStr[256];
		sprintf_s(dataStr, "%02x %02x %02x %02x -- %02x %02x %02x %02x -- %02x %02x %02x %02x -- %02x\n", 
			pData[0], pData[1], pData[2], pData[3], pData[4], pData[5], pData[6], pData[7], pData[8], pData[9], pData[10], pData[11], pData[12]);
