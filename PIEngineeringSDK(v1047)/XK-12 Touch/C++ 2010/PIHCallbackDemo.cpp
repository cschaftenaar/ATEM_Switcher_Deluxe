...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_BL2:
			//Sets the backlighting intensities, one value for all bank 1 and for all bank 2
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
		...
		case IDC_DESCRIPTOR:
			...
			SendMessage(GetDlgItem(hwndDlg, CHK_CHANGE), BM_SETCHECK, BST_UNCHECKED, 0);
			if (hDevice == -1) return TRUE;
        ...
        case IDC_FORCEDATA:
			...
			//turn on callback in case it was turned off
			SendMessage(GetDlgItem(hwndDlg, CHK_NONE), BM_SETCHECK, BST_CHECKED, 0);
		    SendMessage(GetDlgItem(hwndDlg, CHK_NEW), BM_SETCHECK, BST_UNCHECKED, 0);
			SendMessage(GetDlgItem(hwndDlg, CHK_CHANGE), BM_SETCHECK, BST_UNCHECKED, 0);
			if (hDevice == -1) return TRUE;
			...
			return TRUE;
		case IDC_CUSTOM:
			//This report available only on v23 firmware and above
		...
		case IDC_NENABLE:
			if (hDevice == -1) return TRUE;
			{
			//Enable native mouse or joystick
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
			...
			//flip
			hList = GetDlgItem(hDialog, CHK_FLIP);
			if (hList == NULL) return TRUE;
			if (SendMessage(hList, BM_GETCHECK, 0, 0)==BST_CHECKED) flip=1;
			
			//suppress
			hList = GetDlgItem(hDialog, CHK_SUPPRESS);
			if (hList == NULL) return TRUE;
			if (SendMessage(hList, BM_GETCHECK, 0, 0)==BST_CHECKED) suppress=2;

			//enable
			hList = GetDlgItem(hDialog, CHK_ENABLE);
			if (hList == NULL) return TRUE;
			if (SendMessage(hList, BM_GETCHECK, 0, 0)==BST_CHECKED) enabled=4;
			
			buffer[2]=mode;
			buffer[3]=(flip | suppress | enabled);  //bit 1=flip, bit 2=suppress tap to click, bit 3=enable native mouse/joystick
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		if ((hidusagepage == 0xC && writelen==36))    
		{
			...
			else    
			{
				...
				if (pid==1074)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Touch, PID=1074 or PID #1");
				}
				else if (pid==1075)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Touch, PID=1075");
				}
				else if (pid==1076)
				{
					AddEventMsg(hDialog, "Found Device: XK-12 Touch, PID=1076 or PID #2");
				}
				else
				{
	...
	AddEventMsg(hDialog, "No X-keys XK-12 Touch device found");
}
//------------------------------------------------------------------------
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
		...
		//Touchpad Data
		//if touchpad in relative mode, 3 bytes then pData[2] will be either 0 or 1
		//z=pData[7] either 0 or 1
        //delta x=pData[8] in mickeys
        //delta y=pData[9] in mickeys
