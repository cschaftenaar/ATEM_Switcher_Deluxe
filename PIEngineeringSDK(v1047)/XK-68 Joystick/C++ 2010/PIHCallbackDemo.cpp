	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1117)
				{
					AddEventMsg(hDialog, "Found Device: XK-68 Joystick, PID=1117 or PID #1");
				}
				else if (pid==1118)
				{
					AddEventMsg(hDialog, "Found Device: XK-68 Joystick, PID=1118");
				}
				else if (pid==1119)
				{
					AddEventMsg(hDialog, "Found Device: XK-68 Joystick, PID=1119 or PID #2");
				}
				else
				{
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
		...
		char dataStr[256];
		sprintf_s(dataStr, "%02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x -- %02x %02x %02x -- %02x %02x %02x %02x\n", 
